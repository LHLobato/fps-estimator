...
from build_db import Users
from email_utils import send_email_login, send_email_recovery, send_email_signup
from fastapi import APIRouter, Depends, HTTPException, status
from fastapi_limiter.depends import RateLimiter
from jose import JWTError, jwt
from passlib.context import CryptContext
from routers.dependencies import get_current_user_id, get_session
from schemas import (
    CodeSchema,
    LoginSchema,
    PasswordReset,
...
async def forgot_password(
    user_schema: UserBase, session: Session = Depends(get_session)
):
    """
...
async def verify_recovery_code(
    code_schema: CodeSchema, session: Session = Depends(get_session)
):
...
async def change_password(new_password: PasswordReset, session=Depends(get_session)):
